...
{
	_tprintf( _T("%s [%s] "), sPrompt, sChoices );
	for (;;) {
		TCHAR const cKey = _gettch();
		if (cKey==VK_ESCAPE) {
			_tprintf( _T("ESC\r\n") );
			return VK_ESCAPE;
		}

		// traverse sChoices string in order to find cKey
		int nChoiceIndex;
		for (nChoiceIndex=0; _T('\0')!=sChoices[nChoiceIndex]; nChoiceIndex++)
			if (bIgnoreCase) {
				if (_totlower(sChoices[nChoiceIndex])==_totlower(cKey)) break;
			} else {
				if (sChoices[nChoiceIndex]==cKey) break;
			}

		// Has ckey been found in sChoices?
		if (_T('\0')==sChoices[nChoiceIndex]) {
			// end of string is reached -> then cKey has not been found
			_tprintf( _T("%c\b\a"), cKey );
		} else {
			// success.
			// Echo and return the character from sChoices, because it could differ from cKey in case (bIgnoreCase).
			_tprintf( _T("%c\r\n"), sChoices[nChoiceIndex] );
			return sChoices[nChoiceIndex];
		}
	}
}
...
LPCTSTR AlpErrorString( LPTSTR buffer, size_t const SizeOfBufferInTchar, long const nAlpResult )
{
	...
	ALP_ERROR_STRING(ALP_DRIVER_VERSION);
	ALP_ERROR_STRING(ALP_SDRAM_INIT);
...
long const gAlpLedTypes[] = {
	// visible mono
	ALP_HLD_PT120_RAX, ALP_HLD_PT120_GREEN, ALP_HLD_PT120TE_BLUE,
	ALP_HLD_PT120_RED, ALP_HLD_PT120_BLUE,
	// ultraviolet
	ALP_HLD_CBT90_UV, ALP_HLD_CBT120_UV,
	ALP_HLD_CBM120_UV365, ALP_HLD_CBM120_UV,
	// visible white
...
LPCTSTR AlpLedTypeName(long const nApiValue)
{
	switch (nApiValue) {
	default: return NULL;
	case ALP_HLD_PT120_RAX: return _T("PT120_RAX");
	case ALP_HLD_PT120_RED: return _T("PT120_RED (to 2016)");
	case ALP_HLD_PT120_GREEN: return _T("PT120_GREEN");
	case ALP_HLD_PT120_BLUE: return _T("PT120_BLUE (to 2013)");
	case ALP_HLD_CBT90_UV: return _T("CBT90_UV");
	case ALP_HLD_CBT120_UV: return _T("CBT120_UV");
	case ALP_HLD_CBM120_UV365: return _T("CBM120_UV365 (365nm peak wavelength, up to 12A)");
	case ALP_HLD_CBM120_UV: return _T("CBM120_UV (380nm and higher)");
...
CConsolePosition::CConsolePosition(void)
{
	CONSOLE_SCREEN_BUFFER_INFO ScreenBufferInfo;
	GetConsoleScreenBufferInfo( GetStdHandle(STD_OUTPUT_HANDLE), &ScreenBufferInfo );
	m_pos = ScreenBufferInfo.dwCursorPosition;
}
void CConsolePosition::Restore() const
{
