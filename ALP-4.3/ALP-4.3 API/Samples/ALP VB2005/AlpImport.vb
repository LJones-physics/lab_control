...
'	In case of trouble, please always read alp.h and ALP high-speed API description
'	first, in order to debug functions, control types, and values!
...
Public Class AlpImport
    ...
    Private Const DllFileName As String = "alp4395.dll"
    ...
    Enum AlpReturnCodes As Int32
		...
		ALP_DRIVER_VERSION = 1019		' Support in ALP drivers missing. Update drivers and power-cycle device.
    ...
    Enum AlpDevTypes As Int32           ' AlpDevInquire and AlpDevControl - ControlTypes
        ...
        ALP_DDC_FPGA_TEMPERATURE = 2050 ' V4100, V4395: DDC FPGAs Temperature Diode
        ALP_APPS_FPGA_TEMPERATURE = 2051 ' V4100, V4395: Application FPGAs Temperature Diode
        ALP_PCB_TEMPERATURE = 2052      ' V4100, V4395: Board temperature (V4395: near SO-DIMM)
    ...
    Enum AlpSeqTypes As Int32           ' AlpSeqInquire and AlpSeqControl - ControlTypes
        ...
        ALP_FLUT_ENTRIES9 = 2120        ' number of FLUT entries; default=1 (supports all values from 1 to ALP_FLUT_MAX_ENTRIES9)
        ALP_FLUT_OFFSET9 = 2122         ' offset of FLUT index; default=0
    End Enum
    ...
    Enum AlpProjTypes As Int32          ' AlpProjInquire and AlpProjControl - ControlTypes
        ...
        ALP_FLUT_WRITE_9BIT = 2325      ' Transfer 9-bit entries to FLUT (AlpProjControlEx)
        ALP_FLUT_WRITE_18BIT = 2326     ' Transfer 18-bit entries to FLUT (AlpProjControlEx)
    End Enum
    ...
    End Structure
    ' Special Type of UserStructPtr, to be used with ControlTypes ALP_DEV_DYN_SYNCH_OUT*_GATE
    <DllImport(DllFileName, CallingConvention:=DllCallingConvention, _
    ...
    EntryPoint:="AlpDevControlEx")> Public Overloads Shared Function AlpDevControlEx _
        (ByVal DeviceId As Int32, ByVal ControlType As Int32, _
        ByVal UserStructPtr As Byte()) As AlpReturnCodes
    End Function
    ...
    ' Type of UserStructPtr depends on ControlType
    <DllImport(DllFileName, CallingConvention:=DllCallingConvention, _
    EntryPoint:="AlpProjControlEx")> Public Overloads Shared Function AlpProjControlEx _
        (ByVal DeviceId As Int32, ByVal ControlType As Int32, _
        ByVal UserStructPtr As Byte()) As AlpReturnCodes
    End Function
    Public Shared Function AlpProjControlEx_FlutWrite( _
        ByVal DeviceId As Int32, _
        ByVal FlutOffset As Int32, _
        ByVal FrameNumbers As Int32(), _
        ByVal FlutWriteType As AlpProjTypes) _
        As AlpReturnCodes

        ' Usage example:
        ' AlpSeqControl(DevId, SeqId, AlpSeqTypes.ALP_FLUT_MODE, AlpSeqValues.ALP_FLUT_9BIT)
        ' AlpSeqControl(DevId, SeqId, AlpSeqTypes.ALP_FLUT_ENTRIES9, 3)
        ' AlpProjControlEx_FlutWrite(DevId, 0, New Int32() {1, 1, 0}, AlpProjTypes.ALP_FLUT_WRITE_9BIT)

        ' The tFlutWrite structure consists of 4-byte values only.
        ' The approach here is to "flatten" this structure to an array and fill the correct indexes.
        Dim FlatStruct As Int32()
        ReDim FlatStruct(FrameNumbers.GetLength(0) - 1 + 2)

        FlatStruct(0) = FlutOffset
        FlatStruct(1) = FrameNumbers.GetLength(0)
        For i As Int32 = 0 To FrameNumbers.GetLength(0) - 1
            FlatStruct(i + 2) = FrameNumbers(i)
        Next i

        If (FlutWriteType = AlpProjTypes.ALP_FLUT_WRITE_9BIT Or FlutWriteType = AlpProjTypes.ALP_FLUT_WRITE_18BIT) Then
            Return AlpProjControlEx(DeviceId, FlutWriteType, FlatStruct)
        Else
            Return AlpReturnCodes.ALP_PARM_INVALID
        End If
    End Function
    ' Int32-Array UserStructPtr suitable e.g. for ControlTypes ALP_FLUT_WRITE_*BIT
    <DllImport(DllFileName, CallingConvention:=DllCallingConvention, _
    EntryPoint:="AlpProjControlEx")> Public Overloads Shared Function AlpProjControlEx _
        (ByVal DeviceId As Int32, ByVal ControlType As Int32, _
        ByVal UserStructPtr As Int32()) As AlpReturnCodes
    End Function
End Class
