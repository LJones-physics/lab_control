...
BEGIN_MESSAGE_MAP(CAlpSampleDoc, CDocument)
	...
	ON_COMMAND(ID_ALP_OPEN, OnFileOpen)
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()
...
void CAlpSampleDoc::OnAlpInit() 
{
	...
	// Ask the ALP API for DMD resolution first:
	if (AlpDevInquire(m_AlpId, ALP_DEV_DISPLAY_WIDTH, &m_nSizeX) == ALP_OK
		&& AlpDevInquire(m_AlpId, ALP_DEV_DISPLAY_HEIGHT, &m_nSizeY) == ALP_OK
		)
	{	// success
		m_bAlpInit = TRUE;
	}
	else if (AlpDevInquire(m_AlpId, ALP_DEV_DMDTYPE, &m_nDmdType) == ALP_OK)
	{	// ALP does not yet support ALP_DEV_DISPLAY_WIDTH and _HEIGHT, so inquire DMD type
		switch (m_nDmdType) {
	...
	// Set up IML
	if ( m_nSizeX>USHRT_MAX || m_nSizeY>USHRT_MAX)
		AfxMessageBox( _T("IML error: out of range") );
	long result = IMLSetImageSize( LOWORD(m_nSizeX), LOWORD(m_nSizeY) );
	if (IML_SUCCESS != result)
		AfxMessageBox( _T("IML error") );
...
void CAlpSampleDoc::OnAlpLoad() 
{
	...
	// show frames without dark phase
//	AlpSeqControl(m_AlpId, m_AlpSeqId[m_AlpSeq], ALP_BIN_MODE, ALP_BIN_UNINTERRUPTED);
...
void CAlpSampleDoc::OnFileOpen() 
{
	...
	if (!(AfxGetApp()->DoPromptFileName(m_strFileName, IDS_OPEN_PATTERN,
			OFN_HIDEREADONLY | OFN_FILEMUSTEXIST, /*bOpenFileDialog=*/ TRUE, NULL)))
		return;

	// allocate ALP sequence memory - 8 bit, 1 frame
	if ( (m_AlpSeq == SEQU_MAX) ||
	     (AlpSeqAlloc(m_AlpId, 8, 1, &m_AlpSeqId[m_AlpSeq]) != ALP_OK) )
	{
		AfxMessageBox(IDS_ERROR_SEQ_ALLOC, MB_OK);
		return;
	}
	...
	char unsigned*	m_pImage = new char unsigned [IMLGetImageSize()];

	// load file - adjust alignment
	long ret = IMLLoadImage( m_strFileName, IML_ALIGN_LEFT|IML_ALIGN_TOP, 0, m_pImage);

	if (IML_SUCCESS != ret)
	{
		AfxMessageBox("File open failed.", MB_OK);
		AlpSeqFree(m_AlpId, m_AlpSeqId[m_AlpSeq]);
		goto OpenFailed;
	}

	// load sequence data into ALP memory
	if (AlpSeqPut(m_AlpId, m_AlpSeqId[m_AlpSeq], 0, 1, m_pImage) != ALP_OK)
	{
		CString str;
		str.Format(IDS_ERROR_SEQ_PUT, m_AlpSeq + 1);
		AfxMessageBox(str, MB_OK);
