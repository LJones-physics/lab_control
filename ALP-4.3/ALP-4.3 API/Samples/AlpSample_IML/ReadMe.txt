...
To simplify load of image files, we provide an extended AlpSample. Loading
of different Image Files (BMP, GIF, JPEG, TIFF ...) is implemented now. 

Our Image Load Library (iml.dll) and Microsoft Windows GDI+ (gdiplus.dll,
included since Windows XP) are used. Both files must reside in the library
search path, e.g. in the application start path.
...
AlpSampleDoc.h, AlpSampleDoc.cpp - the document
	...
	ALP menu commands. 
	NEW: Loading of Image Files is implemented now.
